...
const HOME_POSITION = new THREE.Vector3(0, 0.2, 4.2);
const MIN_DISTANCE = 0.6;
const MAX_DISTANCE = 40;

export interface OrbSceneOptions {
  pureTransparent?: boolean;
}
...
export function createOrbScene(container: HTMLElement, options?: OrbSceneOptions): OrbSceneApi {
  ...
  camera.position.copy(options?.pureTransparent ? new THREE.Vector3(0, 0, 5.2) : HOME_POSITION);
  ...
  const bloom = new UnrealBloomPass(
    ...
    0.05, // strength - clean, sharp lines, minimal glow
    0.02, // radius - tight sharp edges
    0.85, // threshold - peak line highlights only
  );
  composer.addPass(bloom);

  // Chromatic aberration + color grade shader (zero blur)
  const chromaticShader = {
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        float alpha = clamp(cg.a + (cr.r + cg.g + cb.b) * 0.8, 0.0, 1.0);
  ...
  // ——— COLORS ———
  const C_BRIGHT = 0xffaa00;
  const C_MID = 0xee8800;
  const C_DIM = 0xaa5500;
  const C_FAINT = 0x663300;
  const C_HOT = 0xffd700;

  // ——— ORB ROOT ———
  const orbGroup = new THREE.Group();
  if (options?.pureTransparent) {
  ...
  // ═══════════════════════════════════════════════
  // REVOLVING GOLDEN HUD RINGS (Matching Reference)
  // ═══════════════════════════════════════════════
  const goldenRingGroup = new THREE.Group();
  orbGroup.add(goldenRingGroup);

  const C_GOLD1 = 0xffd700; // Bright Gold
  const C_GOLD2 = 0xffaa00; // Amber Gold
  const C_GOLD3 = 0xff7700; // Hot Orange

  function makeThickRing(radius: number, color: number, opacity = 1.0, count = 4, spread = 0.025) {
    const group = new THREE.Group();
    for (let i = 0; i < count; i++) {
      const offset = (i - (count - 1) / 2) * spread;
      const ringGeo = latRing(radius + offset, 0, 180);
      const ringLine = new THREE.Line(ringGeo, lineMat(color, opacity * (1 - Math.abs(offset) * 2)));
      group.add(ringLine);
    }
    return group;
  }

  // Ring 1 — Outer prominent vertical gold band
  const gRing1 = makeThickRing(2.0, C_GOLD1, 1.0, 5, 0.025);
  gRing1.rotation.x = Math.PI * 0.45;
  goldenRingGroup.add(gRing1);

  // Ring 2 — Outer prominent horizontal equatorial gold band
  const gRing2 = makeThickRing(2.0, C_GOLD2, 0.95, 5, 0.025);
  gRing2.rotation.x = Math.PI * 0.05;
  goldenRingGroup.add(gRing2);

  // Ring 3 — Inclined diagonal gold band
  const gRing3 = makeThickRing(2.05, C_GOLD3, 0.85, 4, 0.025);
  gRing3.rotation.x = -Math.PI * 0.35;
  gRing3.rotation.z = Math.PI * 0.2;
  goldenRingGroup.add(gRing3);

  // Ring 4 — Inner core surrounding ring band
  const gRing4 = makeThickRing(1.15, C_GOLD1, 0.9, 3, 0.02);
  gRing4.rotation.x = Math.PI * 0.2;
  goldenRingGroup.add(gRing4);
  ...
  // Zero background fog or text noise — purely sharp rings and middle core
  const textOuter = scatterText(0, () => 0.04, () => R1, [0, 0]);
  const textInner = scatterText(0, () => 0.03, () => R3, [0, 0]);
  const textAmbient = scatterText(0, () => 0.03, () => R3, [0, 0]);
  ...
  function animate() {
    ...
    coreSphereMat.opacity = Math.min(0.3, coreOpacity);
